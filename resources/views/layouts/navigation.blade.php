<nav x-data="{ open: false }" class="main-nav-default py-2 md:py-3 mb-1 w-full" id="mainNav">
                ...
                <x-dropdown.dropdown-flowbite :trigger="Auth::user()->name ?? 'Welcome, Guest'" iconTrigger="user">
                    <li class="w-full">
                        ...
                        <a href="{{ url('/my-profile') }}"
                            class="rounded-lg px-4 py-3 text-sm hover:bg-white-owl dark:hover:bg-orochimaru hover:text-hot-shot dark:hover:text-eerie-black inline-flex justify-start items-center w-full">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Super admin menu -->
            @if (Auth::user() && Auth::user()->isSuperAdmin())
            @include('layouts.responsive-superadmin-menu')
            @endif

            <!-- Owner menu -->
            @if (Auth::user() && Auth::user()->isVenueManagement())
            @include('layouts.responsive-owner-menu')
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                ...
                @auth
                <div class="font-medium text-sm text-after-midnight dark:text-white">{{ Auth::user()->name }}</div>
                <div class="font-medium text-xs text-after-midnight/80 dark:text-white-owl">{{ Auth::user()->email }}
                </div>
                @endauth
                ...
                @guest
                <div class="font-medium text-sm text-after-midnight dark:text-white">Welcome, Guest!</div>
                <div class="font-medium text-xs text-after-midnight/80 dark:text-white-owl">Please sign in or sign up to
                    continue.</div>
                @endguest
